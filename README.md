...
一致で以下のような操作を判定するだけの決定的なロジックです。それでも
「自然言語 → 構造化アクション → 検証 → 適用」という一連のパイプラインと、
AIが人間と同じ操作の幅を持てることを一通り確認できます。

| 指示の例 | 実行されるアクション |
|---|---|
| 「〇〇を完了にして」/「〇〇を進行中に」 | 該当タスクをreparent |
| 「〇〇を優先にして」 | `priority: high` を設定(UI上に⭐表示) |
| 「〇〇を削除して」 | 該当タスクをdelete-node |
| 「レビュー列を追加して」 | 新しい列(Column)をcreate-node |
| 「〇〇を△△に変更して」 | タスク名をupdate-property |
| 「タスクを追加して: A、B、C」 | 複数タスクを一括create-node |
| 「To Doを並び替えて」 | 列内のタスクを五十音順にmove-nodeで並び替え |
| 「全部完了にして」 | 全タスクを一括でDoneへreparent(最大12件) |
| 「Doneを空にして」 | 該当列の全タスクを一括delete-node |
| (どれにも一致しない自由文) | 指示文そのものを新しいタスクとしてTo Doへ追加 |

`AI_MODE=live` にすると、これと同じアクション語彙を実際のLLM(Anthropic API)に
渡して自由な自然言語から生成させることもできます(`ai/AnthropicLLMProvider.js`)。
...
列間の移動も、列内の並び替えも、HTML5のドラッグ&ドロップで行います
(セレクトボックスや↑↓ボタンではありません)。ドロップ位置に応じて
挿入位置を示すプレースホルダーが表示され、離した位置に応じて
`reparent` (列をまたぐ場合のみ) → `move-node` (正確な挿入位置) の順で
送信されます。
...
- AIへの自然言語指示から、reparent/create-node/update-property/move-node/
  delete-nodeのすべての操作が、人間の操作と同じ検証・ブロードキャスト経路で
  実行されること(列作成・優先マーク・リネーム・複数タスク一括追加・
  一括完了・列内並び替え・列クリアをそれぞれ実WebSocket接続で確認済み)
